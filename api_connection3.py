...
def loop_downloads(already_dowloaded_files):
    for y in years:
        for f in fealds:
            category = y + f
            if category in already_dowloaded_files:
                continue
            string_try_again = "again"
            while string_try_again == "again":
                string_try_again = search_concept_by_feald_or_year(link = url, university_user = "UNIVERSITY_USER", university_password = "",personal_mail = "", personal_password = "", type_feald = f, year = y)

def simple_try(f, y):
    string_try_again = search_concept_by_feald_or_year(link = url, university_user = "", university_password = "",personal_mail = "", personal_password = "", type_feald = f, year = y)

def already_dowloaded(path_files):
    list_already_downloaded = []
    for filename in os.listdir(path_files):
        print(filename[:4], filename[5:9])
        list_already_downloaded.append(filename[5:9] + filename[:4])
    print(list_already_downloaded)
    return list_already_downloaded

def run():
    already_dowloaded_files = already_dowloaded("C:/Users/joaqu/OneDrive/Documentos/drive/relaves/raw_data/")
    loop_downloads(already_dowloaded_files=already_dowloaded_files)
